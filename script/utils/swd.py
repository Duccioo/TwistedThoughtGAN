...
# https://arxiv.org/pdf/1706.02631.pdf
import numpy as np
import torch
...
def pyramid_down(image, device="cpu"):
    ...
    multiband = [
        F.conv2d(image[:, i : i + 1, :, :], gaussian_k, padding=2, stride=2)
        for i in range(1)
    ]
...
def pyramid_up(image, device="cpu"):
    ...
    multiband = [
        F.conv2d(upsample[:, i : i + 1, :, :], gaussian_k, padding=2) for i in range(1)
    ]
...
def extract_patches(
    pyramid_layer, slice_indices, slice_size=7, unfold_batch_size=128, device="cpu"
):
    ...
    for i in range(n):
        ...
        x = (
            pyramid_layer[ind_start:ind_end]
            .unfold(2, slice_size, 1)
            .unfold(3, slice_size, 1)
            .reshape(
    ...
    x = (x - mean) / (std + 1e-8)
    # reshape to 2rank
    x = x.reshape(-1, 1 * slice_size * slice_size)
    return x
...
def swd(
    ...
    with torch.no_grad():
        ...
        pyramid1 = minibatch_laplacian_pyramid(
            image1, n_pyramids, pyramid_batchsize, device=device
        )
        pyramid2 = minibatch_laplacian_pyramid(
            image2, n_pyramids, pyramid_batchsize, device=device
        )
        result = []
        ...
        for i_pyramid in range(n_pyramids + 1):
            ...
            p1 = extract_patches(
                pyramid1[i_pyramid], indices, slice_size=slice_size, device="cpu"
            )
            p2 = extract_patches(
                pyramid2[i_pyramid], indices, slice_size=slice_size, device="cpu"
            )
